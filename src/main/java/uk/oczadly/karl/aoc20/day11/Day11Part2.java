package uk.oczadly.karl.aoc20.day11;
...
public class Day11Part2 {
    ...
    public static void main(String[] args) {
        SeatMap seatMap = SeatMap.load(Helper.loadInput(11));
        while (seatMap.nextIteration());
        
        int occupied = (int)Arrays.stream(seatMap.seatData).flatMap(Arrays::stream)
                .filter(s -> s == SeatState.SEAT_OCCUPIED)
                .count();
    
        System.out.printf("Number of occupied seats = %d%n", occupied);
    }
    ...
    static class SeatMap {
        ...
        private SeatState nextIteration(int row, int col) {
            ...
            if (state != SeatState.FLOOR) {
                ...
                for (int r = -1; r <= 1; r++) {
                    for (int c = -1; c <= 1; c++) {
                        if (hasNeighbour(row, col, r, c)) neighbours++;
                    }
                }
                // Mutate
        ...
        private boolean hasNeighbour(int row, int col, int deltaRow, int deltaCol) {
            if (deltaRow == 0 && deltaCol == 0) return false;
            int r = row + deltaRow, c = col + deltaCol;
            while (r >= 0 && r < height && c >= 0 && c < width) {
                SeatState state = getSeat(r, c);
                if (state != SeatState.FLOOR)
